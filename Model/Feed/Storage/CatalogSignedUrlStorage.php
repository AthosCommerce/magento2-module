...
class CatalogSignedUrlStorage implements CatalogStorageInterface
{
    /**
     * @var FormatterPool
     */
    ...
    {
        $format = MetadataInterface::FORMAT_CSV;
        if (!$format) {
    ...
    public function commit(int $id, bool $deleteFile = true): void
    {
        ...
        $urlPath = parse_url($this->specification->getCatalogPreSignedUrl(), PHP_URL_PATH);

        // For txt.gz treat as TXT format for compression
        if ($urlPath && (strpos($urlPath, MetadataInterface::FORMAT_TXT_GZ) !== false)) {
