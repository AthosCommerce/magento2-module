...
try {
    $product = $productRepository->get('athoscommerce_simple_diff_price_1', false, null, true);
...
try {
    $product = $productRepository->get('athoscommerce_simple_diff_price_2', false, null, true);
    $productRepository->delete($product);
    $productIds[] = $product->getId();
    $criteria = $stockStatusCriteriaFactory->create();
    $criteria->setProductsFilter($product->getId());
