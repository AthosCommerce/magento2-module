...
/**
 * Integration test for grouped parent save through catalog_product_save_after.
 *
 * Verifies that saving a grouped parent updates both:
 ...
 */
class GroupedUpdateObserverTest extends TestCase
{
    use IndexingEntitiesTrait;

    private const SITE_ID_PREFIX = 'test-grouped-update-observer-';
    ...
    /**
     * Saving a grouped parent must route both the parent and linked child
     * indexing entities to the same next_action/is_indexable outcome.
     *
     * @dataProvider groupedProductSaveDataProvider
     * @magentoConfigFixture current_store athoscommerce/indexing/enable_live_indexing 1
     */
    public function testExecute_OnGroupedProductSave_UpdatesParentAndChildIndexingEntities(
        int $status,
        int $visibility,
    ...
    /**
     * When live indexing is disabled the grouped parent save must leave both the
    ...
    public function groupedProductSaveDataProvider(): array
    {
        return [
    ...
    private function createAndSaveGroupedProduct(int $status, int $visibility): array
    {
        ...
        $childProduct->setTypeId(Type::TYPE_SIMPLE)
            ...
            ->setSku('athos_grouped_child_' . uniqid('', true))
            ->setPrice(10)
        ...
        $parentProduct->setTypeId(Grouped::TYPE_CODE)
            ...
            ->setSku('athos_grouped_parent_' . uniqid('', true))
            ->setStatus($status)
