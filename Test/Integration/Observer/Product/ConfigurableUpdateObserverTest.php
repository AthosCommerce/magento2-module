...
/**
 * Integration test for configurable parent save through catalog_product_save_after.
 *
 * Verifies that saving a configurable parent updates both:
 ...
 */
class ConfigurableUpdateObserverTest extends TestCase
{
    use IndexingEntitiesTrait;

    private const SITE_ID_PREFIX = 'test-configurable-update-observer-';
    ...
    /**
     * Saving a configurable parent must route both the parent and linked child
     * indexing entities to the same next_action/is_indexable outcome.
     *
     * Because UpdateObserver uses forceIndexable=true the UPSERT path always
     * sets is_indexable=true; on the DELETE path the standard
     * SetIndexingEntitiesToDeleteAction rules apply.
     *
     * @dataProvider configurableProductSaveDataProvider
     * @magentoConfigFixture current_store athoscommerce/indexing/enable_live_indexing 1
     */
    public function testExecute_OnConfigurableProductSave_UpdatesParentAndChildIndexingEntities(
        int    $status,
        int    $visibility,
    ...
    ): void
    {
        ...
        $this->assertSame(
            $expectedIsIndexable,
            $parentEntity->getIsIndexable(),
            'Parent is_indexable mismatch'
        );
    ...
    /**
     * When live indexing is disabled saving a configurable parent must leave both
    ...
    public function configurableProductSaveDataProvider(): array
    {
        return [
            ...
            'disabled_not_visible_last_no_action_expects_no_action' => [
                'status' => Status::STATUS_DISABLED,
                'visibility' => Visibility::VISIBILITY_NOT_VISIBLE,
                'initialLastAction' => Actions::NO_ACTION,
                'initialIsIndexable' => true,
    ...
    private function createAndSaveConfigurableProduct(int $status, int $visibility): array
    {
        ...
        $childProduct->setTypeId(Type::TYPE_SIMPLE)
            ...
            ->setSku('athos_cfg_child_' . uniqid('', true))
            ->setPrice(10)
        ...
        $parentProduct->setTypeId(Configurable::TYPE_CODE)
            ...
            ->setSku('athos_cfg_parent_' . uniqid('', true))
            ->setStatus($status)
     ...
     * @return IndexingEntity|null
     */
    private function getIndexingEntityByTargetIdAndParentId(
        int  $targetId,
        ?int $targetParentId
    ): ?IndexingEntity
    {
